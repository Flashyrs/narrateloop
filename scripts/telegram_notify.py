...
# def already_running():
#     current_pid = os.getpid()
#     script_name = "telegram_notify.py"
#     for proc in psutil.process_iter(['pid', 'cmdline']):
#         try:
#             if (proc.info['pid'] != current_pid and 
#                 proc.info['cmdline'] and 
#                 script_name in proc.info['cmdline'][-1]):
#                 return True
#         except Exception:
#             continue
#     return False
...
# def init_startup_logs():
#     log("✅ Script started (Task Scheduler)", telegram=True)
#     send_telegram_log("✅ NarrateLoopBot started via Task Scheduler")
...
#             if now != last_sent:
#                 if now == "10:00":
#                     send_telegram_log("⏰ Auto upload trigger: /upload 1")
#                     requests.post(
#                         f"https://api.telegram.org/bot{os.getenv('TELEGRAM_BOT_TOKEN')}/sendMessage",
#                         data={"chat_id": os.getenv("TELEGRAM_CHAT_ID"), "text": "/upload 1"}
#                     )
#                 elif now == "16:00":
#                     send_telegram_log("⏰ Auto upload trigger: /upload 2")
#                     requests.post(
#                         f"https://api.telegram.org/bot{os.getenv('TELEGRAM_BOT_TOKEN')}/sendMessage",
#                         data={"chat_id": os.getenv("TELEGRAM_CHAT_ID"), "text": "/upload 2"}
#                     )
#                 elif now == "21:00":
#                     send_telegram_log("⏰ Auto upload trigger: /upload 4")
#                     requests.post(
#                         f"https://api.telegram.org/bot{os.getenv('TELEGRAM_BOT_TOKEN')}/sendMessage",
#                         data={"chat_id": os.getenv("TELEGRAM_CHAT_ID"), "text": "/upload 4"}
#                     )
#                 last_sent = now
#             time.sleep(30)
...
# def load_pipeline():
#     from main_pipeline import run_pipeline, run_pipeline_upload_specific, get_upload_status
#     return run_pipeline, run_pipeline_upload_specific, get_upload_status
...
#         is_running = True
#         await safe_reply(update, "Starting processing...")
#         run_pipeline, _, _ = load_pipeline()
...
#         is_running = True
#         await safe_reply(update, f"Uploading video for story_{story_num}.json...")
#         _, run_pipeline_upload_specific, _ = load_pipeline()
...
#         try:
#             date_arg = context.args[0] if context.args else datetime.now().strftime("%Y%m%d")
#             _, _, get_upload_status = load_pipeline()
#             status_text = get_upload_status(date_arg)
#             await safe_reply(update, status_text)
#         except Exception as e:
#             await safe_reply(update, f"Error fetching status: {e}")
...
#     def startup_run():
#         try:
#             log("Starting processing in background...")
#             run_pipeline, _, _ = load_pipeline()
#             Thread(target=run_pipeline, kwargs={"upload": False}, daemon=True).start()
#             log("Pipeline launched. Waiting for commands...")
#             send_telegram_log("🤖 Bot is now running! Type /help")
#         except Exception as e:
#             log(f"Startup pipeline error: {e}")
#             with open("startup_debug.log", "a", encoding="utf-8") as f:
#                 f.write(f"Startup error: {e}\n")
