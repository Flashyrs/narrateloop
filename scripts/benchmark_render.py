...
def run_test(name, cmd_args, out_file):
    print(f"\n==========================================")
    print(f"STARTING {name}")
    print(f"==========================================")
    full_cmd = ["/usr/bin/time", "-v"] + cmd_args
    t0 = time.time()
    p = subprocess.run(full_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    t1 = time.time()
    elapsed = t1 - t0
    ...
    for line in stderr_lines:
        if "User time (seconds):" in line:
            cpu_user = line.split(":")[-1].strip()
        elif "System time (seconds):" in line:
            cpu_sys = line.split(":")[-1].strip()
    ...
    size_mb = os.path.getsize(out_file) / (1024.0 * 1024.0) if os.path.exists(out_file) else 0.0
    print(f"[SUCCESS] {name} completed in {elapsed:.2f}s ({elapsed/60.0:.2f} min)")
    print(f"  -> Wall-clock: {elapsed:.2f}s, CPU User: {cpu_user}s, Sys: {cpu_sys}s, CPU %: {cpu_percent}")
    print(f"  -> Max RSS: {max_rss} KB, File Size: {size_mb:.2f} MB")
    return {
        "name": name,
        "elapsed": elapsed,
        "ok": True,
...
def benchmark_test_3():
    ...
    cmd = ["ffmpeg", "-benchmark", "-y"] + g_inputs
    ...
    return run_test("TEST 3 (Gameplay + Title + ONE Dialogue Layer + Subs + Audio)", cmd, out_file)

# ----------------------------------------------------
# TEST 4: Gameplay + Title + Audio (WITHOUT Subtitles to isolate Libass cost)
# ----------------------------------------------------
def benchmark_test_4():
    out_file = os.path.join(SCRATCH_DIR, "bench_t4.mp4")
    g_inputs, g_filter, num_g = build_gameplay_inputs_and_filters()
    card_idx = num_g
    voice_idx = num_g + 1
    music_idx = num_g + 2
    ...
    v_f = [
        g_filter,
        f"[{card_idx}:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,format=yuva420p,fade=t=out:st={title_end_time-0.35:.2f}:d=0.35:alpha=1[card]",
        f"[gameplay][card]overlay=0:0:enable='between(t,0,{title_end_time:.2f})':eof_action=pass[v_out]"
    ]
    a_f = [
        f"[{voice_idx}:a]volume=0.220,asplit=2[v_main][v_sc];"
        f"[{music_idx}:a]atrim=0:{audio_dur:.2f},asetpts=PTS-STARTPTS,volume=0.030[m_vol];"
    ...
    return run_test("TEST 4 (Gameplay + Title + Audio - NO SUBTITLES)", cmd, out_file)

if __name__ == "__main__":
    t3_res = benchmark_test_3()
    t4_res = benchmark_test_4()
    
    print("\n" + "="*60)
    print("EMPIRICAL BENCHMARK SUMMARY (PHASE 1)")
    print("="*60)
    print(f"Test 1 (Baseline: Gameplay + Card + Subs + Audio):")
    print(f"  -> CPU Time: 339.0s (~5.65 min), Wall-Clock: ~780s (~13 min), Size: 87.1 MB")
    print(f"Test 2 (16 PNG Overlays):")
    print(f"  -> CPU Time: 625.0s (~10.42 min), Max RSS: ~371 MB (3.59 GB VSZ, 2.08 GB Swap), Wall-Clock: ~51 min")
    print(f"Test 3 (1 Consolidated Dialogue Timeline Stream):")
    print(f"  -> Wall-Clock: {t3_res['elapsed']:.2f}s, CPU User: {t3_res['cpu_user']}s, Sys: {t3_res['cpu_sys']}s, Max RSS: {t3_res['max_rss_kb']} KB, Size: {t3_res['size_mb']:.2f} MB")
    print(f"Test 4 (No Subtitles - Libass Cost Isolation):")
    print(f"  -> Wall-Clock: {t4_res['elapsed']:.2f}s, CPU User: {t4_res['cpu_user']}s, Sys: {t4_res['cpu_sys']}s, Max RSS: {t4_res['max_rss_kb']} KB, Size: {t4_res['size_mb']:.2f} MB")
    print("="*60)
